...
## H3 Resolutions in Practice

The pipeline materializes a fixed trio of resolutions (`h3_r7`, `h3_r8`, `h3_r9`) on every run so downstream jobs can pick the scale they need without recomputing.

| Resolution | Hexagon Size | Chicago Coverage | Typical Use |
|------------|--------------|------------------|-------------|
| **7** | ~1.22 km | ~2,778 hexagons | District-level summaries |
| **8** | ~461 m | ~15,161 hexagons | Neighborhood comparisons |
| **9** | ~174 m | ~85,431 hexagons | Street-level analysis |

*Counts measured from 2018–2025 historical data.*

CLI reminder: `python src/l2_features.py [start_year]` processes every `data/l1/year=YYYY` directory where `YYYY >= start_year`. Resolution selection is automatic—no CLI switch required.
...
### **Materialized H3 Resolutions & Schema**
- L2 now materializes a small set of H3 resolutions by default: `h3_r7`, `h3_r8`, and `h3_r9` to support multi-scale aggregation in L3 without recomputing H3 repeatedly.
- Parquet outputs are written with a deterministic pyarrow schema and include provenance metadata (`h3_resolutions`, `producer`) to ensure downstream tools can read partitions reliably.
